...
public class CreateOrderRequestVo {
    ...
    @Schema(description = "商品简单描述，该字段须严格按照规范传递", required = true)
    private String body;
    ...
    @Schema(description = "必须传正确的用户端IP,支持ipv4、ipv6格式，获取方式详见获取用户ip指引", required = true)
    ...
    @Schema(description = "JSAPI -JSAPI支付（或小程序支付）， NATIVE -Native支付， APP -APP支付，MWEB--H5支付", required = true)
