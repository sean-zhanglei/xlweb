package com.nbug.depends.websockets.core.session;
...
public class WebSocketSessionManagerImpl implements WebSocketSessionManager {
    ...
    public void addSession(WebSocketSession session) {
        ...
        // 添加到 userSessions 中
        LoginUserVo user = WebSocketFrameworkUtils.getLoginUser(session);
        if (user == null) {
            return;
        }
        ...
        CopyOnWriteArrayList<WebSocketSession> sessions = userSessionsMap.get(Long.valueOf(user.getUser().getId()));
        if (sessions == null) {
            sessions = new CopyOnWriteArrayList<>();
            if (userSessionsMap.putIfAbsent(Long.valueOf(user.getUser().getId()), sessions) != null) {
                sessions = userSessionsMap.get(Long.valueOf(user.getUser().getId()));
            }
        }
    ...
    public void removeSession(WebSocketSession session) {
        ...
        if (CollUtil.isEmpty(sessions)) {
            userSessionsMap.remove(Long.valueOf(user.getUser().getId()), sessions);
        }
    }
