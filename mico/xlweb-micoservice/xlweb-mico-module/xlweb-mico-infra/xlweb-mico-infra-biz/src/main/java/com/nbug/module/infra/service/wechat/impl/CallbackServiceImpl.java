...
import com.nbug.mico.common.vo.CallbackVo;
...
public class CallbackServiceImpl implements CallbackService {
    ...
    public String weChat(String xmlInfo) {
        ...
        try{
            ...
            //解析xml
            CallbackVo callbackVo = XlwebUtil.mapToObj(map, CallbackVo.class);
            AttachVo attachVo = JSONObject.toJavaObject(JSONObject.parseObject(callbackVo.getAttach()), AttachVo.class);

            //判断openid
            User user = userApi.getById(attachVo.getUserId()).getCheckedData();
            if (ObjectUtil.isNull(user)) {
                //用户信息错误
                throw new XlwebException("用户信息错误！");
            }
            ...
            if (!Constants.SERVICE_PAY_TYPE_ORDER.equals(attachVo.getType()) && !Constants.SERVICE_PAY_TYPE_RECHARGE.equals(attachVo.getType())) {
                ...
                throw new XlwebException("未知的支付类型！");
            }
            // 订单
            if (Constants.SERVICE_PAY_TYPE_ORDER.equals(attachVo.getType())) {
                ...
                StoreOrder storeOrder = storeOrderApi.getInfoByEntity(orderParam).getCheckedData();
                if (ObjectUtil.isNull(storeOrder)) {
                    logger.error("wechat pay error : 订单信息不存在==》" + callbackVo.getOutTradeNo());
                    throw new XlwebException("wechat pay error : 订单信息不存在==》" + callbackVo.getOutTradeNo());
                }
                ...
                if (ObjectUtil.isNull(wechatPayInfo)) {
                    ...
                    throw new XlwebException("wechat pay error : 微信订单信息不存在==》" + callbackVo.getOutTradeNo());
                }
            ...
            if (Constants.SERVICE_PAY_TYPE_RECHARGE.equals(attachVo.getType())) {
                ...
                if (!rechargePayAfter) {
                    ...
                    throw new XlwebException("wechat pay error : 数据保存失败==》" + callbackVo.getOutTradeNo());
                }
            }
            sb.append("<return_code><![CDATA[SUCCESS]]></return_code>");
            sb.append("<return_msg><![CDATA[OK]]></return_msg>");
    ...
    public String weChatRefund(String xmlInfo) {
        ...
        StoreOrder storeOrder = storeOrderApi.getByOderId(outRefundNo).getCheckedData();
        if (ObjectUtil.isNull(storeOrder)) {
        ...
        boolean update = storeOrderApi.updateById(storeOrder).getCheckedData();
        if (update) {
            // 退款task
    ...
    private String getSignKey(String appid) {
        String publicAppid = configApi.getValueByKey(Constants.CONFIG_KEY_PAY_WE_CHAT_APP_ID).getCheckedData();
        String miniAppid = configApi.getValueByKey(Constants.CONFIG_KEY_PAY_ROUTINE_APP_ID).getCheckedData();
        String appAppid = configApi.getValueByKey(Constants.CONFIG_KEY_PAY_WE_CHAT_APP_APP_ID).getCheckedData();
        String signKey = "";
        if (StrUtil.isBlank(publicAppid) && StrUtil.isBlank(miniAppid) && StrUtil.isBlank(appAppid)) {
            throw new XlwebException("pay_weixin_appid或pay_routine_appid不能都为空");
        }
        ...
        if (StrUtil.isNotBlank(appAppid) && appid.equals(appAppid)) {
            signKey = configApi.getValueByKeyException(Constants.CONFIG_KEY_PAY_WE_CHAT_APP_APP_KEY).getCheckedData();
        }
        return signKey;
    }
