package com.nbug.module.order.util;
    ...
    public Boolean checkPayType(String payType){
        ...
        switch (payType){
            case PayConstants.PAY_TYPE_WE_CHAT:
                result = "1".equals(configApi.getValueByKey(SysConfigConstants.CONFIG_PAY_WEIXIN_OPEN).getCheckedData());
                break;
            case PayConstants.PAY_TYPE_YUE:
                result = ("1".equals(configApi.getValueByKey(SysConfigConstants.CONFIG_YUE_PAY_STATUS).getCheckedData()));
                break;
            case PayConstants.PAY_TYPE_ALI_PAY:
                result = ("1".equals(configApi.getValueByKey(SysConfigConstants.CONFIG_ALI_PAY_STATUS).getCheckedData()));
                break;
        }
        return result;
    }
...
//                queryWrapper.eq(StoreOrder::getShippingType, 1);
    ...
    public String getStoreNameAndCarNumString(int orderId){
        List<StoreOrderInfoOldVo> currentOrderInfo = storeOrderInfoApi.getOrderListByOrderId(orderId).getCheckedData();
