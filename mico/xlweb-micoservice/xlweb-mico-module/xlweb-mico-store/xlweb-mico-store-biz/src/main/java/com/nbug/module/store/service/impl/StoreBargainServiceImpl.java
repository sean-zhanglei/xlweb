...
import com.nbug.mico.common.constants.BargainConstants;
...
import com.nbug.module.store.dal.StoreBargainDao;
    ...
    */
    ...
    public boolean saveBargain(StoreBargainRequest request) {
        ...
        if (CollUtil.isEmpty(request.getAttrValue())) {
            throw new XlwebException("请选择砍价商品的规格属性");
        }
        ...
        if (ObjectUtil.isNull(attrValueRequest.getQuota()) || attrValueRequest.getQuota() <= 0) {
            throw new XlwebException("活动限购数量必须大于0");
        }
        // 可砍价的金额
        ...
        if (tempPrice.compareTo(multiply) < 0) {
            ...
            throw new XlwebException("必须保证每个人最少砍1分钱");
        }
        ...
        bargain.setId(null);
        // 头图、轮播图
        bargain.setImage(attachmentApi.clearPrefix(request.getImage()).getCheckedData());
        bargain.setImages(attachmentApi.clearPrefix(request.getImages()).getCheckedData());
        // 活动开始结束时间
        ...
        singleAttrValue.setImage(attachmentApi.clearPrefix(singleAttrValue.getImage()).getCheckedData());
    ...
    public boolean updateBargain(StoreBargainRequest request) {
        ...
        if (saveCount <= 0) {
            throw new XlwebException("编辑砍价商品失败");
        }
        ...
        attrValue.setImage(attachmentApi.clearPrefix(attrValueRequest.getImage()).getCheckedData());
    ...
    public StoreProductInfoResponse getAdminDetail(Integer bargainId) {
        ...
        if (ObjectUtil.isNull(storeBargain) || storeBargain.getIsDel()) {
            throw new XlwebException("未找到对应砍价商品信息");
        }
    ...
    public BargainDetailH5Response getH5Detail(Integer id) {
        ...
        if (!storeBargain.getStatus()) {
            throw new XlwebException("砍价商品已下架");
        }
        ...
        if (CollUtil.isEmpty(storeProductAttrValuesBargain)) {
            throw new XlwebException("砍价商品规格属性值未找到");
        }
    ...
    public MyRecord start(BargainFrontRequest request) {
        ...
        if (ObjectUtil.isNull(storeBargain) || storeBargain.getIsDel()) {
            throw new XlwebException("对应的砍价商品不存在");
        }
        if (!storeBargain.getStatus()) {
            throw new XlwebException("砍价商品已下架");
        }
        if (storeBargain.getQuota() <= 0 || storeBargain.getStock() <= 0) {
            throw new XlwebException("砍价商品已售罄");
        }
        ...
        if (CollUtil.isNotEmpty(historyList)) {
            ...
            if (CollUtil.isNotEmpty(collect)) {
                throw new XlwebException("请先完成当前砍价活动");
            }
            // 判断是否达到参与砍价活动上限
            if (historyList.size() >= storeBargain.getNum()) {
                throw new XlwebException("您已达到当前砍价活动上限");
            }
        }
        ...
        if (!save) {
            throw new XlwebException("参与砍价失败");
        }
    ...
    public void stopAfterChange() {
        ...
        if (!b) {
            ...
            throw new XlwebException("砍价活动结束后更新用户状态失败");
        }
    }
    ...
    public StoreBargain getByIdException(Integer id) {
        ...
        if (ObjectUtil.isNull(storeBargain)) throw new XlwebException("砍价商品不存在或未开启");
        return storeBargain;
    }
    ...
    public Boolean operationStock(Integer id, Integer num, String type) {
        ...
        if (!update) {
            throw new XlwebException("更新砍价商品库存失败,商品id = " + id);
        }
        return update;
    }
    ...
    public BargainHeaderResponse getHeader() {
        ...
        HashMap<Integer, User> userMap = userApi.getMapListInUid(uidList).getCheckedData();
