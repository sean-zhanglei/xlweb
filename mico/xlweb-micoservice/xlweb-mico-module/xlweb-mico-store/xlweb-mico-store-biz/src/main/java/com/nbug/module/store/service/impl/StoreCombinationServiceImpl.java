...
import com.nbug.mico.common.response.CombinationDetailH5Response;
...
import com.nbug.mico.common.response.StorePinkResponse;
...
import com.nbug.module.store.dal.StoreCombinationDao;
...
//        clearNotCheckedAndValidationPrice(request);
        ...
        if (stopTime <= System.currentTimeMillis()) {
            throw new XlwebException("活动结束时间不能小于当前时间");
        }

        storeCombination.setId(null);
        // 头图、轮播图
        storeCombination.setImage(attachmentApi.clearPrefix(request.getImage()).getCheckedData());
        storeCombination.setImages(attachmentApi.clearPrefix(request.getImages()).getCheckedData());
        // 活动开始结束时间
    ...
    public Boolean deleteById(Integer id) {
        ...
        if (combination.getIsShow().equals(true) && combination.getStartTime() <= timeMillis && timeMillis <= combination.getStopTime()) {
            throw new XlwebException("活动开启中，商品不支持删除");
        }
    ...
    public Boolean updateCombination(StoreCombinationRequest request) {
        ...
        if (existCombination.getIsShow().equals(true) && existCombination.getStartTime() <= timeMillis && timeMillis <= existCombination.getStopTime()) {
            throw new XlwebException("活动开启中，商品不支持修改");
        }
...
//        storeProductResponse.setStopTimeStr(DateUtil.timestamp2DateStr(storeCombination.getStopTime(), Constants.DATE_FORMAT_DATE));
    ...
    public Boolean updateCombinationShow(Integer id, Boolean isShow) {
        ...
        if (isShow) {
            ...
            if (ObjectUtil.isNull(product)) {
                throw new XlwebException("关联的商品已删除，无法开启活动");
            }
        }
    ...
    public CombinationDetailResponse getH5Detail(Integer comId) {
        ...
        if (ObjectUtil.isNull(storeCombination) || storeCombination.getIsDel()) {
            throw new XlwebException("对应拼团商品不存在");
        }
        if (!storeCombination.getIsShow()) {
            throw new XlwebException("拼团商品已下架");
        }
...
//        List<ProductAttrResponse> skuAttr = getSkuAttr(attrList);
        ...
        if (CollUtil.isNotEmpty(headList)) {
            List<StorePinkResponse> headPinkList = headList.stream().map(i -> {
                ...
                User teamUser = userApi.getById(i.getUid()).getCheckedData();
    ...
    public GoPinkResponse goPink(Integer pinkId) {
        ...
        if (ObjectUtil.isNull(teamPink) || teamPink.getIsRefund()) throw new XlwebException("对应的拼团不存在");
        StoreCombination storeCombination = getById(teamPink.getCid());
        if (ObjectUtil.isNull(storeCombination) || storeCombination.getIsDel()) throw new XlwebException("对应拼团商品不存在");

        // 判断拼团活动时效
        if (!storeCombination.getIsShow()) {
            throw new XlwebException("拼团活动已结束");
        }
        ...
        for (StorePink pink : pinkList) {
            if (pink.getKId().equals(0)) {
                ...
                if (pink.getUid().equals(user.getUid())) {
                    ...
                    User teamUser = userApi.getById(pink.getUid()).getCheckedData();
            ...
            User userOne = userApi.getById(pink.getUid()).getCheckedData();
    ...
    public Boolean removePink(StorePinkRequest storePinkRequest) {
        ...
        if (ObjectUtil.isNull(userPink)) throw new XlwebException("未查到拼团信息，无法取消");
        if (userPink.getIsRefund()) throw new XlwebException("拼团订单已退款");
        // 获取是否拼团成功
        if (userPink.getStatus() == 2) {
            throw new XlwebException("拼团已完成，无法取消");
        }
        ...
        if (userPink.getStatus() == 3) {
            throw new XlwebException("拼团已申请取消");
        }

        StoreOrder order = storeOrderService.getByOderId(userPink.getOrderId());
        if (ObjectUtil.isNull(order) || order.getIsDel()) throw new XlwebException("拼团订单不存在");
        if (order.getStatus() == -1 && order.getRefundStatus() != 0) {
            throw new XlwebException("拼团订单已进入退款流程");
        }
        ...
        boolean apply = orderApi.refundApply(refundRequest).getCheckedData();
        if (!apply) throw new XlwebException("订单申请退款失败");
    ...
    public StoreCombination getByIdException(Integer combinationId) {
        ...
        if (ObjectUtil.isNull(storeCombination)) throw new XlwebException("拼团商品不存在或未开启");
        return storeCombination;
    }
    ...
    public Boolean operationStock(Integer id, Integer num, String type) {
        ...
        if (!update) {
            throw new XlwebException("更新拼团商品库存失败,商品id = " + id);
        }
        return update;
    }
    ...
    public CombinationIndexResponse getIndexInfo() {
        ...
        combinationList.forEach(e -> {
            int percentIntVal = XlwebUtil.percentInstanceIntVal(e.getQuota(), e.getQuotaShow());
    ...
    public CombinationHeaderResponse getHeader() {
        ...
        // 获取拼团列表banner
        List<HashMap<String, Object>> bannerList = systemGroupDataApi.getListMapByGid(Constants.GROUP_DATA_ID_COMBINATION_LIST_BANNNER).getCheckedData();
