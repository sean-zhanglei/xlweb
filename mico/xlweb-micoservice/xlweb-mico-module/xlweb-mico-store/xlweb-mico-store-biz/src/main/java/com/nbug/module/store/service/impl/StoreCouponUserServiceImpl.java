...
import com.nbug.module.store.dal.StoreCouponUserDao;
...
public class StoreCouponUserServiceImpl extends ServiceImpl<StoreCouponUserDao, StoreCouponUser> implements StoreCouponUserService {
    ...
    */
    ...
    public PageInfo<StoreCouponUserResponse> getList(StoreCouponUserSearchRequest request, PageParamRequest pageParamRequest) {
        ...
        HashMap<Integer, User> userList = userApi.getMapListInUid(uidList).getCheckedData();
    ...
    public Boolean receive(StoreCouponUserRequest request) {
        ...
        List<Integer> uidList = XlwebUtil.stringToArray(request.getUid());
        if (uidList.size() < 1) {
            throw new XlwebException("请选择用户！");
        }
        ...
        if (storeCoupon.getIsLimited()) {
            ...
            if (storeCoupon.getLastTotal() < uidList.size()) {
                throw new XlwebException("当前剩余数量不够领取！");
            }
        }
        ...
        if (uidList.size() < 1) {
            //都已经领取过了
            throw new XlwebException("当前用户已经领取过此优惠券了！");
        }
    ...
    public List<StoreCouponUserOrder> getListByPreOrderNo(String preOrderNo) {
        ...
        lambdaQueryWrapper.eq(StoreCouponUser::getUid, userApi.getUserIdException().getCheckedData());
    ...
    public void overdueTask() {
        ...
        if (!update) throw new XlwebException("批量更新优惠券过期动作失败");
    }
