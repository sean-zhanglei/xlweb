...
import com.nbug.module.store.dal.StoreProductReplyDao;
    ...
    */
    ...
    public PageInfo<StoreProductReplyResponse> getList(StoreProductReplySearchRequest request, PageParamRequest pageParamRequest) {
        ...
        for (StoreProductReply productReply : dataList) {
            ...
            productReplyResponse.setPics(XlwebUtil.stringToArrayStr(productReply.getPics()));
    ...
    public Boolean create(StoreProductReplyAddRequest request) {
        ...
        if (ObjectUtil.isNull(storeOrder) || !storeOrder.getUid().equals(user.getUid())) {
            throw new XlwebException("该订单不存在");
        }
        ...
        storeProductReply.setAvatar(attachmentApi.clearPrefix(user.getAvatar()).getCheckedData());
        ...
        if (StringUtils.isNotBlank(request.getPics())) {
            String pics = request.getPics().replace("[\"","").replace("\"]","")
                    .replace("\"","");
            storeProductReply.setPics(attachmentApi.clearPrefix(ArrayUtils.toString(pics)).getCheckedData());
        }
        ...
        if (!execute) {
            throw new XlwebException("评价订单失败");
        }
        return execute;
    }
    ...
    public boolean virtualCreate(StoreProductReplyAddRequest request) {
        ...
        storeProductReply.setAvatar(attachmentApi.clearPrefix(storeProductReply.getAvatar()).getCheckedData());
        storeProductReply.setUnique(XlwebUtil.randomCount(11111,9999)+"");
    ...
    public ProductDetailReplyResponse getH5ProductReply(Integer proId) {
        ...
        // 评价图
        productReplyResponse.setPics(XlwebUtil.stringToArrayStr(storeProductReply.getPics()));
        // 昵称
    ...
    private Integer checkIsReply(StoreProductReply storeProductReply) {
        ...
        if (null == orderInfoVoList || orderInfoVoList.size() < 1) {
            throw new XlwebException("没有找到商品信息");
        }
...
//            Integer productId = orderInfoVo.getInfo().getInteger("product_id");
        ...
        if (replyCount > 0) {
            throw new XlwebException("该商品已评价");
        }
