...
import com.nbug.mico.common.response.SecKillDetailH5Response;
...
import com.nbug.module.store.dal.StoreSeckillDao;
...
//        String currentSkillTime = null;
        ...
        for (StoreSeckill product : storeProducts) {
            ...
            storeProductResponse.setImages(XlwebUtil.stringToArrayStr(product.getImages()));
    ...
    public Boolean saveSeckill(StoreSeckillAddRequest request) {
        request.getAttrValue().forEach(e -> {
            if ((ObjectUtil.isNull(e.getQuota()) || e.getQuota() <= 0)) {
                throw new XlwebException("请正确输入限量");
            }
        });

        if (isExistTile(request.getTitle())) {
            throw new XlwebException("活动标题已经存在");
        }

        StoreSeckill storeSeckill = new StoreSeckill();
        BeanUtils.copyProperties(request, storeSeckill);
        // 主图
        storeSeckill.setImage(attachmentApi.clearPrefix(storeSeckill.getImage()).getCheckedData());
        // 轮播图
        storeSeckill.setImages(attachmentApi.clearPrefix(storeSeckill.getImages()).getCheckedData());
        // 设置秒杀开始时间和结束时间
    ...
    public Boolean updateSeckill(StoreSeckillAddRequest request) {
        ...
        if (ObjectUtil.isNull(storeSeckill) || storeSeckill.getIsDel()) {
            throw new XlwebException("秒杀商品不存在");
        }

        if (storeSeckill.getStatus().equals(1)) {
            throw new XlwebException("请先关闭秒杀商品，再修改商品信息");
        }
        ...
        //主图
        seckill.setImage(attachmentApi.clearPrefix(seckill.getImage()).getCheckedData());

        //轮播图
        seckill.setImages(attachmentApi.clearPrefix(seckill.getImages()).getCheckedData());
    ...
    public StoreSeckillDetailResponse getDetailH5(Integer skillId) {
        ...
        if (ObjectUtil.isNull(storeSeckill) || storeSeckill.getIsDel()) {
            throw new XlwebException("未找到对应秒杀商品信息");
        }
        if (storeSeckill.getStatus().equals(0)) {
            throw new XlwebException("秒杀商品已下架");
        }
    ...
    public List<StoreSecKillH5Response> getKillListByTimeId(String timeId, PageParamRequest pageParamRequest) {
        ...
        storeSeckills.forEach(e->{
            ...
            response.setPercent(XlwebUtil.percentInstanceIntVal(e.getQuotaShow() - e.getQuota(), e.getQuotaShow()));
    ...
    public StoreSeckill getByIdException(Integer id) {
        ...
        lqw.eq(StoreSeckill::getIsShow, true);
        StoreSeckill storeSeckill = dao.selectOne(lqw);
        if (ObjectUtil.isNull(storeSeckill) || storeSeckill.getIsDel()) throw new XlwebException("秒杀商品不存在或以删除");
        return storeSeckill;
    }
    ...
    public Boolean operationStock(Integer id, Integer num, String type) {
        ...
        if (!update) {
            throw new XlwebException("更新商品库存失败！商品id = " + id);
        }
        return update;
    }
