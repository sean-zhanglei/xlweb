package com.nbug.module.store.util.delete;
...
import com.nbug.mico.common.utils.UrlUtil;
...
//        JSONObject tbJsonData = new JSONObject(JSONExample.tbJson); // just Test
        ...
        if (null == data) throw new XlwebException("复制商品失败--返回数据格式错误--未找到data");
        JSONObject item = data.getJSONObject("item");
        if (null == item) throw new XlwebException("复制商品失败--返回数据格式错误--未找到item");
...
//        if (null == props) throw new XlwebException("复制商品失败--返回数据格式错误--未找到props");
...
//        JSONObject tbJsonData = new JSONObject(JSONExample.snJson); // just Test
    ...
    public void setConfig(String url, int tag){
        ...
        switch (tag){ // 导入平台1=淘宝，2=京东，3=苏宁，4=拼多多， 5=天猫
            case 1:
                baseUrl = configApi.getValueByKey("importProductTB").getCheckedData();
                rightEndUrl += UrlUtil.getParamsByKey(url, "id");
                break;
            case 2:
                baseUrl = configApi.getValueByKey("importProductJD").getCheckedData();
                rightEndUrl += url.substring(url.lastIndexOf("/")+1).replace(".html","");
                break;
            case 3:
                baseUrl = configApi.getValueByKey("importProductSN").getCheckedData();
            ...
            case 4:
                ...
                baseUrl = configApi.getValueByKey("importProductPDD").getCheckedData();
                break;
            case 5:
                rightEndUrl += UrlUtil.getParamsByKey(url, "id");
                baseUrl = configApi.getValueByKey("importProductTM").getCheckedData();
                break;
        }
//        String token = configApi.getValueByKey("importProductToken");
        String token = configApi.getValueByKey("copy_product_apikey").getCheckedData();
        if(StringUtils.isBlank(token)){
            throw new XlwebException("请配置复制产品平台的Token -- www.99api.com");
        }
        if(StringUtils.isBlank(baseUrl)){
            throw new XlwebException("请配置复制产品平台的Url-- www.99api.com");
        }
//        rightUrl = "?apikey="+configApi.getValueByKey("importProductToken")+rightEndUrl;
        rightUrl = "?apikey="+configApi.getValueByKey("copy_product_apikey")+rightEndUrl;
    ...
    public HashMap<Integer, ProductActivityItemResponse> getActivityByProduct(Integer productId, String activity){
        ...
        List<Integer> activitys = XlwebUtil.stringToArrayInt(activity);
