...
public interface CategoryApi {
    ...
    public CommonResult<List<Category>> getByIds(@RequestParam List<Integer> adminIdList);

    @GetMapping(PREFIX + "/getChildVoListByPid")
    @Operation(summary = "根据父级id获取子级分类集合")
    @Parameter(name = "pid", description = "父级分类ID", required = true)
    public CommonResult<List<Category>> getChildVoListByPid(@RequestParam Integer pid);

    @GetMapping(PREFIX + "/getListInId")
    @Operation(summary = "根据id获取分类集合")
    @Parameter(name = "cateIdList", description = "分类ID集合", required = true)
    public CommonResult<HashMap<Integer, String>> getListInId(@RequestParam List<Integer> cateIdList);
    ...
    public CommonResult<List<CategoryTreeVo>> getListTree(@RequestParam Integer type,
