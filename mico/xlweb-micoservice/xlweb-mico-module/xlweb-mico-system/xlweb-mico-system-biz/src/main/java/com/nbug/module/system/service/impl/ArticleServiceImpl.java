...
import com.nbug.module.system.dal.ArticleDao;
    ...
    */
    ...
    public ArticleResponse getVoByFront(Integer id) {
        ...
        if (ObjectUtil.isNull(article)) {
            throw new XlwebException("文章不存在");
        }
    ...
    public Boolean create(ArticleRequest articleRequest) {
        ...
        article.setImageInput(attachmentApi.clearPrefix(article.getImageInput()).getCheckedData());
        article.setContent(attachmentApi.clearPrefix(article.getContent()).getCheckedData());
        article.setVisit("0");
        return save(article);
    }
    ...
    public Boolean deleteById(Integer id) {
        Article article = getById(id);
        if (ObjectUtil.isNull(article)) {
            throw new XlwebException("文章已删除");
        }
