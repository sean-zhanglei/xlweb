...
import com.nbug.mico.common.vo.SystemConfigFormItemConfigRegListVo;
import com.nbug.mico.common.vo.SystemConfigFormItemVo;
import com.nbug.mico.common.vo.SystemConfigFormVo;
import com.nbug.module.system.dal.SystemFormTempDao;
    ...
    */
    ...
    public void checkForm(SystemFormCheckRequest systemFormCheckRequest) {
        ...
        try {
            ...
            throw new XlwebException("模板表单 【" + formTemp.getName() + "】 的内容不是正确的JSON格式！");
        }
        ...
        for (String item : systemConfigFormVo.getFields()) {
            ...
            if(systemConfigFormItemVo.get__config__().getRequired() && "".equals(map.get(model))) {
                throw new XlwebException(systemConfigFormItemVo.get__config__().getLabel() + "不能为空！");
            }
            //正则验证
    ...
    public Boolean add(SystemFormTempRequest systemFormTempRequest) {
        try {
            ...
            throw new XlwebException("模板表单 【" + systemFormTempRequest.getName() + "】 的内容不是正确的JSON格式！");
        }
        ...
        if(regList.size() > 0) {
            for (SystemConfigFormItemConfigRegListVo systemConfigFormItemConfigRegListVo : regList) {
                if(!ValidateFormUtil.regular(value, name, systemConfigFormItemConfigRegListVo.getPattern())) {
                    throw new XlwebException(systemConfigFormItemConfigRegListVo.getMessage());
                }
            }
        }
    }
