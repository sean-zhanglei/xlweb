...
public class UserServiceImpl extends ServiceImpl<UserDao, User> implements UserService {
    ...
    public PageInfo<UserResponse> getList(UserSearchRequest request, PageParamRequest pageParamRequest) {
        ...
        if (!StringUtils.isBlank(request.getLabelId())) {
            String tagIdSql = XlwebUtil.getFindInSetSql("u.tag_id", request.getLabelId());
        ...
        for (User user : userList) {
            ...
            userResponse.setPhone(XlwebUtil.maskMobile(userResponse.getPhone()));
    ...
    public Boolean updateIntegralMoney(UserOperateIntegralMoneyRequest request) {
        ...
        if (request.getMoneyType().equals(2) && request.getMoneyValue().compareTo(BigDecimal.ZERO) != 0) {
            if (user.getNowMoney().subtract(request.getMoneyValue()).compareTo(BigDecimal.ZERO) < 0) {
                throw new XlwebException("余额扣减后不能小于0");
            }
        }
        ...
        if (!execute) {
            throw new XlwebException("修改积分/余额失败");
        }
        return execute;
    }
    ...
    public Boolean group(String id, String groupIdValue) {
        if (StrUtil.isBlank(id)) throw new XlwebException("会员编号不能为空");
        if (StrUtil.isBlank(groupIdValue)) throw new XlwebException("分组id不能为空");

        //循环id处理
        List<Integer> idList = XlwebUtil.stringToArray(id);
        idList = idList.stream().distinct().collect(Collectors.toList());
        List<User> list = getListInUid(idList);
        if (CollUtil.isEmpty(list)) throw new XlwebException("没有找到用户信息");
        if (list.size() < idList.size()) {
            throw new XlwebException("没有找到用户信息");
        }
        for (User user : list) {
    ...
    public Boolean password(PasswordRequest request) {
        ...
        //密码
        user.setPwd(XlwebUtil.encryptPassword(request.getPassword(), user.getAccount()));
    ...
    public User getInfoException() {
        User user = getInfo();
        if (user == null) {
            throw new XlwebException("用户信息不存在！");
        }

        if (!user.getStatus()) {
            throw new XlwebException("用户已经被禁用！");
        }
        return user;
    }
    ...
    public Boolean updatePhoneVerify(UserBindingPhoneUpdateRequest request) {
        ...
        if (!user.getPhone().equals(request.getPhone())) {
            throw new XlwebException("手机号不是当前用户手机号");
        }
    ...
    public Boolean updatePhone(UserBindingPhoneUpdateRequest request) {
        ...
        if (null != user) {
            throw new XlwebException("此手机号码已被注册");
        }
    ...
    public UserCenterResponse getUserCenter() {
        ...
        if (ObjectUtil.isNull(currentUser)) {
            throw new XlwebException("您的登录已过期，请先登录");
        }
        UserCenterResponse userCenterResponse = new UserCenterResponse();
        BeanUtils.copyProperties(currentUser, userCenterResponse);
        // 优惠券数量
        userCenterResponse.setCouponCount(storeCouponUserApi.getUseCount(currentUser.getUid()).getCheckedData());
        // 收藏数量
        userCenterResponse.setCollectCount(storeProductRelationApi.getCollectCountByUid(currentUser.getUid()).getCheckedData());

        userCenterResponse.setVip(false);
        if (userCenterResponse.getLevel() > 0) {
            SystemUserLevel systemUserLevel = systemUserLevelApi.getByLevelId(currentUser.getLevel()).getCheckedData();
    ...
    public Boolean tag(String id, String tagIdValue) {
        if (StrUtil.isBlank(id)) throw new XlwebException("会员编号不能为空");
        if (StrUtil.isBlank(tagIdValue)) throw new XlwebException("标签id不能为空");
    ...
    public User registerPhone(String phone, Integer spreadUid) {
        ...
        if (!execute) {
            throw new XlwebException("创建用户失败!");
        }
        return user;
    }
    ...
    public Object getInfoByCondition(Integer userId, Integer type, PageParamRequest pageParamRequest) {
        switch (type) {
            case 0:
                return storeOrderApi.findPaidListByUid(userId, pageParamRequest).getCheckedData();
            case 1:
            ...
            case 3:
                ...
                scur.setUid(userId);
                return storeCouponUserApi.findListByUid(userId, pageParamRequest).getCheckedData();
            case 4:
    ...
    public TopDetail getTopDetail(Integer userId) {
        ...
        topDetail.setMothConsumeCount(storeOrderApi.getSumPayPriceByUidAndDate(userId, Constants.SEARCH_DATE_MONTH).getCheckedData());
        topDetail.setAllConsumeCount(storeOrderApi.getSumPayPriceByUid(userId).getCheckedData());
        topDetail.setMothOrderCount(storeOrderApi.getOrderCountByUidAndDate(userId, Constants.SEARCH_DATE_MONTH).getCheckedData());
        topDetail.setAllOrderCount(storeOrderApi.getOrderCountByUid(userId).getCheckedData());
        return topDetail;
    }
    ...
    public User registerByThird(RegisterThirdUserRequest thirdUserRequest) {
        User user = new User();
        user.setAccount(DigestUtils.md5Hex(XlwebUtil.getUuid() + DateUtil.getNowTime()));
    ...
    public PageInfo<SpreadOrderResponse> getOrderListBySpreadLevel(RetailShopStairUserRequest request, PageParamRequest pageParamRequest) {
        ...
        List<SpreadOrderResponse> responseList = recordPageInfo.getList().stream().map(e -> {
            ...
            StoreOrder storeOrder = storeOrderApi.getByOderId(e.getLinkId()).getCheckedData();
...
//        // 获取推广人订单号集合
//        List<StoreOrder> orderList = storeOrderApi.getOrderListStrByUids(userIds, request);
...
//    }
    ...
    public void bindSpread(Integer spreadUid) {
        ...
        User user = getInfo();
        if (ObjectUtil.isNull(user)) {
            throw new XlwebException("当前用户未登录,请先登录");
        }
    ...
    public Boolean editSpread(UserUpdateSpreadRequest request) {
        ...
        if (userId.equals(spreadUid)) {
            throw new XlwebException("上级推广人不能为自己");
        }
    ...
    public Boolean updateUserPhone(Integer id, String phone) {
        ...
        if (ObjectUtil.isNull(user)) {
            throw new XlwebException("对应用户不存在");
        }
        if (phone.equals(user.getPhone())) {
            throw new XlwebException("手机号与之前一致");
        }
    ...
    public Boolean updateUserLevel(UpdateUserLevelRequest request) {
        ...
        if (user.getLevel().equals(request.getLevelId())) {
            throw new XlwebException("用户等级与修改前相同");
        }

        SystemUserLevel systemUserLevel = systemUserLevelApi.getByLevelId(request.getLevelId()).getCheckedData();
        if (ObjectUtil.isNull(systemUserLevel)) {
            throw new XlwebException("系统会员等级不存在，请先配置");
        }
        ...
        if (ObjectUtil.isNull(userLevel)) {
            throw new XlwebException("用户会员等级不存在，请检查用户数据");
        }
        if (userLevel.getGrade() > systemUserLevel.getGrade()) {
            if (ObjectUtil.isNull(request.getIsSub())) {
                throw new XlwebException("降低用户等级时，请选择是否扣除用户经验值");
            }
        }
