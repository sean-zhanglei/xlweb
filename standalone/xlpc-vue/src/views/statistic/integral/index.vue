...
import { getPointBasic, getPointTrend, getPointChannel, getPointType } from '@/api/statistic';
import { formatDate } from '@/utils/validate';
import echartsFrom from '@/components/echarts/new';
...
export default {
  ...
  methods: {
    ...
    getPointBasic() {
      getPointBasic(this.formValidate).then((res) => {
        ...
        this.cardLists.map((i, index) => {
          i.count = res[arr[index]];
        });
      });
    },
    getPointChannel() {
      this.loading = true;
      getPointChannel(this.formValidate).then((res) => {
        this.infoList = res;
    ...
    getPointType() {
      this.loading2 = true;
      getPointType(this.formValidate).then((res) => {
        this.infoList2 = res;
        this.tabList2 = res.list;
        this.loading2 = false;
      });
    },
    // 具体日期
    ...
    getPointTrend() {
      ...
      getPointTrend(this.formValidate)
        .then(async (res) => {
          let legend = res.series.map((item) => {
            return item.name;
          });
          let xAxis = res.xAxis;
