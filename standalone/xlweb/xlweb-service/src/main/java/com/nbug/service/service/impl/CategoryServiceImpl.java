...
import com.nbug.common.request.CategoryRequest;
import com.nbug.common.request.CategorySearchRequest;
import com.nbug.service.dao.CategoryDao;
...
public class CategoryServiceImpl extends ServiceImpl<CategoryDao, Category> implements CategoryService {
    ...
    private void updatePidStatusById(Integer id) {
        ...
        List<Integer> categoryIdList = XlwebUtil.stringToArrayByRegex(category.getPath(), "/");
    ...
    public int delete(Integer id) {
        ...
        if(getChildCountByPid(id) > 0){
            throw new XlwebException("当前分类下有子类，请先删除子类！");
        }
    ...
    public Boolean create(CategoryRequest categoryRequest) {
        ...
        if(checkName(categoryRequest.getName(), categoryRequest.getType()) > 0){
            throw new XlwebException("此分类已存在");
        }
        Category category = new Category();
