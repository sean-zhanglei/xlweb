...
import com.nbug.common.response.BargainRecordResponse;
import com.nbug.common.response.BargainUserInfoResponse;
...
import com.nbug.common.model.bargain.StoreBargainUserHelp;
import com.nbug.common.request.StoreBargainUserSearchRequest;
import com.nbug.common.response.StoreBargainUserResponse;
import com.nbug.common.model.order.StoreOrder;
import com.nbug.common.model.user.User;
import com.nbug.service.dao.StoreBargainUserDao;
    ...
    */
    ...
    private BargainUserInfoResponse otherBargainActivity(BargainFrontRequest bargainFrontRequest) {
        ...
        if (bargainUser.getUid().equals(user.getUid())) {// 自己的砍价活动
            ...
            percent =  XlwebUtil.percentInstanceIntVal(alreadyPrice, alreadyPrice.add(surplusPrice));
            infoResponse.setBargainStatus(bargainStatus);
            infoResponse.setAlreadyPrice(alreadyPrice);
            infoResponse.setSurplusPrice(surplusPrice);
            infoResponse.setBargainPercent(percent);
    ...
    private BargainUserInfoResponse oneselfBargainActivity(BargainFrontRequest bargainFrontRequest) {
        ...
        if (storeBargainUser.getStatus().equals(2)) {// 活动结束，砍价未完成
            throw new XlwebException("未在活动期内完成砍价");
        }
        // 参与过该商品的砍价
