...
import com.nbug.common.request.CartNumRequest;
import com.nbug.common.request.CartRequest;
import com.nbug.common.request.CartResetRequest;
...
import com.nbug.common.response.CartInfoResponse;
import com.github.pagehelper.Page;
import com.github.pagehelper.PageHelper;
import com.github.pagehelper.PageInfo;
import com.nbug.common.utils.RedisUtil;
import com.nbug.common.model.cat.StoreCart;
...
import com.nbug.service.dao.StoreCartDao;
    ...
    */
    ...
    public String saveCate(CartRequest storeCartRequest) {
        ...
        if (ObjectUtil.isNull(product) || product.getIsDel() || !product.getIsShow()) {
            throw new XlwebException("未找到对应商品");
        }
        ...
        if (CollUtil.isEmpty(attrValues)) {
            throw new XlwebException("未找到对应的商品SKU");
        }
        ...
        if (existCarts.size() > 0) { // 购物车添加数量
            ...
            if (!updateResult) throw new XlwebException("添加购物车失败");
            ...
            if (dao.insert(storeCart) <= 0) throw new XlwebException("添加购物车失败");
    ...
    public Boolean resetCart(CartResetRequest resetRequest) {
        ...
        if (ObjectUtil.isNull(storeCart)) throw new XlwebException("购物车不存在");
        if (ObjectUtil.isNull(resetRequest.getNum()) || resetRequest.getNum() <= 0 || resetRequest.getNum() >= 999)
            throw new XlwebException("数量不合法");
        ...
        if (!updateResult) throw new XlwebException("重选添加购物车失败");
    ...
    public Boolean updateCartNum(Integer id, Integer number) {
        if (ObjectUtil.isNull(number)) throw new XlwebException("商品数量不合法");
        if (number <=0 || number > 99) throw new XlwebException("商品数量不能小于1大于99");
        StoreCart storeCart = getById(id);
        if (ObjectUtil.isNull(storeCart)) throw new XlwebException("当前购物车不存在");
